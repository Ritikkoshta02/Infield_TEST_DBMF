...
class Biochip_V:
    ...
    def show_mixing_graph(self):
        ...
        string = 'digraph "DD" { \n node [fontsize = 8];\n' 
        fp.write(string)
        for e in self.mixing_graph.edges_iter():
            fp.write(str(e[0])+' -> '+str(e[1])+' [arrowhead = open, arrowsize = .5];\n')
            if self.mixing_graph.node[e[1]]['type'] in ['output','waste'] and e[1] not in op_waste_node:
                op_waste_node.append(e[1])
        
        for v in self.mixing_graph.nodes_iter():
            if self.mixing_graph.node[v]['type'] not in ['reagent','intermediate','output','waste']:
                print ('show_mixing_graph: Invalid type of node')
                sys.exit()
            
            if self.mixing_graph.node[v]['type'] == 'reagent':
                #fp.write('%d [label = "%s\\n %s",  shape = octagon]\n' %(v,self.mixing_graph.node[v]['sample_name'],str(self.mixing_graph.node[v]['conc'])))
                fp.write('%d [label = "%s",  shape = octagon]\n' %(v,self.mixing_graph.node[v]['sample_name']))
                reagent_node.append(v)
            elif self.mixing_graph.node[v]['type'] == 'intermediate':
                ...
                fp.write('%d [label = "s=%d e=%d", shape = oval]\n' %(v,self.mixing_graph.node[v]['s_time'],self.mixing_graph.node[v]['e_time']))
            elif self.mixing_graph.node[v]['type'] == 'waste' and v in op_waste_node:
                fp.write('%d [label="waste",fontcolor=white,width=.3,shape = invhouse,fontsize = 12, style=filled, fillcolor=black]\n' %v)
                        
            elif self.mixing_graph.node[v]['type'] == 'output' and v in op_waste_node:
