...
class ToolTip( Toplevel ):
    ...
    def __init__( self, wdgt, msg=None, msgFunc=None):
        ...
        Toplevel.__init__( self, self.parent, bg='black', padx=1, pady=1 )      # Initalize the Toplevel
        ...
        Message( self, text="self.msgVar", bg='#FFFFDD',aspect=1000 ).grid()  
    ...
    def move( self, event ):
        ...
        """
        print 'In move'+str(random.random())
...
# Try to call the message function.  Will not change the message if the message function is None or the message function fails
    ...
    def hide( self, event=None ):
        ...
        Arguments:
          event: The event that called this function
        """
        print 'In hide'+str(random.random())
        self.withdraw()
...
def print_time():
    ...
    timeString += '%.2f' % ( t%60 )
    print 'In Msg function ->'+ timeString
    return timeString
...
#    HH:MM:SS.00
#    """
#    print 'In Msg function'+str(random.random())
#    t = time()
#    timeString = 'time='
#    timeString += strftime( '%H:%M:', localtime(t) )
#    timeString += '%.2f' % ( t%60, )
...
def main():
    ...
    for (i,j) in range2d( 6, 4 ):
        ...
        if j % 2 == 0:
            ...
            msgFunc = print_time
            text += 'Static Message'
        btnList.append( Canvas( root,height=50, width=50 ) )
