...
outputLocationList = [] # Store output location **************It's 1 index***********************
MixingDroplets = [] #Store list of droplet Ids that are part of same mixier
OutPutTime = []	#To store the list of timestamp that occure output operation
__Id = 1
#========================================================
	...
	for line in ip_f:
		...
		for tk in token:
			...
			if tk[0:3] == 'dis':
				...
				#tempDropletIdForMixing = []
				#Id = droplet[(clock, x1, y1)]
				#tempDropletIdForMixing.append(Id)
				#Id = droplet[(clock, x2, y2)]
				#tempDropletIdForMixing.append(Id)
				#Copy ending
				while m != 0:
					...
					if r1 == r2:	#If mixing size is 4 and vertical then r1 == r2
						...
						d_new = D
						D = D + 1
						#tempDropletIdForMixing.append(d_new)
						...
						l = l + 1
					m = m - 1
				#MixingDroplets.append(tempDropletIdForMixing)
			elif tk[0:3] == 'was':
				i = 6
				r = 0; c = 0
				while tk[i] != ',':
				...
				DisappearenceList.append((clock, r - 1, c - 1))
...
#==================================================================================     
	global row, col, TotalDroplet, MixingDroplets
	...
	for x in range(1, row + 1):
		for y in range(1, col + 1):
			...
			'''
			#M1 and M2 can't be part of same mixer
			M1 = -1
			for d in range(1, TotalDroplet + 1):
				for tuple in MixingDroplets:
					for (idx, temp_d) in enumerate(tuple):
						if temp_d == d:
							M1 = idx
							break
...
#=============================================================================== 
	global dropletDispenserDict, TotalDroplet, T
	x = p[0] + 1
	y = p[1] + 1
...
def  main():
	...
	for tuple in DisappearenceList:
		...
		#if [x + 1, y + 1] not in outputLocationList:
	...
	for (x, y) in DispanceDic:
		...
		for t in DispanceDic[(x, y)]:
			d = droplet[(t, x, y)]
			clauseFile.write("s.add(a_"+str(x + 1)+"_"+str(y + 1)+"_"+str(d)+"_"+str(t)+")\n")
