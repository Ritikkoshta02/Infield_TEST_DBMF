...
def  main():
	...
	for mix in ActiveMixer:
		cycle = (mix[1] - mix[0] + 1) // 6
		l= 0
		cp.append(mix[0] - 1)
		mixingCP.append(mix[0] - 1)
		mixi += 1
		while l != cycle:
			cp.append(mix[0] + 1 + l*6)
			cp.append(mix[0] + 3 + l*6)
			cp.append(mix[0] + 4 + l*6)
			cp.append(mix[0] + 6 + l*6)
			mixingCP.append(mix[0] + 1 + l*6)
	...
	'''
	vulnerablePair.sort(key=lambda x: x[1])
	Timestamp = []
	end = -1
	...
	print("Number of Checkpoint for mixing " + str(mixi))
	print("Number of Checkpoint for disappeance " + str(disa))
	print("Number of Checkpoint for routing " + str(len(rou)))
	print("Number of Checkpoint for routing " + str(len(cp) - disa))
	print("Number of Checkpoint for overlapping routing and dispense " + str(len(roumix)))
