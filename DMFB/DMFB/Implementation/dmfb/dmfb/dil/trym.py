...
subprocess.call('sed "s/_/ /g" op_correct | grep " = True"| sed "s/ = True//g" | sort -nk 5 > golden.txt', shell=True)
#sed 's/_/ /g' op_correct | sed 's/ = True//g' | sort -nk 5 > dil_golden.txt
with open("golden.txt") as csvfile:
...
print(mixdict)
lnglst=[]
subprocess.call('cat cplist.txt | sed "s/,/ /g" | sort -nk 3 > cplistcopy.txt', shell=True)
t=6
			...
			if row[3] in kgd:
				...
				for k,v in kgd.items():
					if(k != row[3] and abs(v[0]-row[0]) + abs(v[1]-row[1]) <= (abs(v[2]-row[2]-2))):
                                	#if(k != row[3] and ((abs(v[0]-x) + abs(v[1]-y) <= (abs(v[2]-row[2])+2)) or (abs(v[0]-row[0]) + abs(v[1]-row[1]) <= (abs(v[2]-row[2])+2)))):
						#(x,y)=tdict[row[2],k]
						print("added row:",row)
						file1.write(str(row).replace("[","").replace("]","").replace(", ",",")+"\n")
						temp=[]
						
						(v,temp) = dpair(k,row[2])
						print(temp)
						if (v==1):
							print("added pair:", temp)
						
							file1.write(str(temp).replace("[","").replace("]","").replace(", ",",")+"\n")
							#prv[row[3]] = (row[0],row[1],row[2])
							prv[k] = (temp[0],temp[1],temp[2])
							#dlst.append(row[3])
							dlst.append(k)
						prv[row[3]] = (row[0],row[1],row[2])
						dlst.append(row[3])
...
file1.close()	
subprocess.call('cat newcplist.txt | sort -u > trycplist.txt', shell=True)
#less newcplist.txt | sort -u > trycplist.txt
