...
def proximityAttack(clauseFile, dropletIdList, step):
#==================================================================================
...
def main(opmil, mixmil, step):
    ...
    with open('cplist.txt') as csvfile:
    ...
    if(len(opmil) > 1):
         constraint = "s.add(sw == Or("
         for x,y,d,t in opmil:
              constraint += "Not(a_%d_%d_%d_%d), "%(x,y,d,t)
         constraint = constraint[:-2] + " ))\n"
         clauseFile.write(constraint)
    elif(len(opmil)==1):
	 for x,y,d,t in opmil:
	      constraint = "s.add(sw == Not(a_%d_%d_%d_%d))\n"%(x,y,d,t)
         clauseFile.write(constraint)

    clauseFile.write("sm = Bool('sm')\n")
    if(len(mixmil) > 1):
         constraint = "s.add(sm == Or("
         for x,y,d,t in mixmil:
              constraint += "Not(a_%d_%d_%d_%d), "%(x,y,d,t)
         constraint = constraint[:-2] + " ))\n"
         clauseFile.write(constraint)
    elif(len(mixmil)==1):
	for x,y,d,t in mixmil:
        ...
        clauseFile.write(constraint) 
    #Output Swap attacks
    clauseFile.write("s.add(Or(sm, sw))\n")
    ...
    #for t in range(0,T+1):
    #proximityAttack(clauseFile, dropletIdList, step)
