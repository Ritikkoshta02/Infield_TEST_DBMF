...
awk '{if(NF != 3) print $1" "$(NF); else print $1" "$3}' t2 | while read d t; do grep "a_[0-9]*_[0-9]*_[0-9]*_"$t" = True" op_correct | sed 's/ = True//g' | sed 's/_/ /g' | awk '{print $2","$3","$5","$4}' >> cplist.txt; done
